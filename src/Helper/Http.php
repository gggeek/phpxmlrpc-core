...
namespace PhpHttpRpc\Helper;

use Psr\Log\LoggerInterface;
use PhpHttpRpc\API\LoggingTrait;
use PhpHttpRpc\API\Exception;

/**
 * @todo do log to php error log as we did before, via the injected logger? (either do it here or leave it to compat classes...)
 */
class Http
{
    use LoggingTrait;
    ...
    public function parseResponseHeaders(&$data, $headersProcessed = false, $debug=0)
    {
        ...
        if (preg_match('/^HTTP\/1\.[0-1] 200 Connection established/', $data)) {
            ...
            if ($bd) {
                ...
                throw Exception::instance(Exception::HTTP_ERROR, '(HTTPS via proxy error, tunnel connection possibly failed)');
            }
        }
        ...
        if (!preg_match('/^HTTP\/[0-9.]+ 200 /', $data)) {
            ...
            throw Exception::instance(Exception::HTTP_ERROR . '(' . $errstr . ')');
        }
        ...
        if (!$headersProcessed) {
            ...
            if (isset($httpResponse['headers']['transfer-encoding']) && $httpResponse['headers']['transfer-encoding'] == 'chunked') {
                if (!$data = Http::decodeChunked($data)) {
                    ...
                    throw Exception::instance(Exception::DECHUNK_FAIL);
                }
            }
                ...
                if ($httpResponse['headers']['content-encoding'] == 'deflate' || $httpResponse['headers']['content-encoding'] == 'gzip') {
                    ...
                    if (function_exists('gzinflate')) {
                        if ($httpResponse['headers']['content-encoding'] == 'deflate' && $degzdata = @gzuncompress($data)) {
                            $data = $degzdata;
                            if ($debug) {
                                $this->debug("---INFLATED RESPONSE---[" . strlen($data) . " chars]---\n$data\n---END---");
                            }
                            ...
                            throw Exception::instance(Exception::DECOMPRESS_FAIL);
                        }
                    } else {
                        //error_log('XML-RPC: ' . __METHOD__ . ': the server sent deflated data. Your php install must have the Zlib extension compiled in to support this.');
                        throw Exception::instance(Exception::CANNOT_DECOMPRESS);
                    }
                }
            }
